...
export function createRegionPageModel(node: RegionNode): RegionPageModel {
  ...
  if (
    !directory ||
    !["local-boundary", "child-region-directory"].includes(directory.id)
  ) {
    throw new Error(`HONHYEOL_MASSAGE_REGION_DIRECTORY_SECTION_MISSING:${node.path}`);
  }
  if (content.hooks.length !== 2 || content.ctaLabels.length !== 3) {
    throw new Error(`HONHYEOL_MASSAGE_REGION_COPY_SHAPE_INVALID:${node.path}`);
  }
  ...
  const galleryHeading =
    children.length > 0
      ...
      : `${node.displayName} 도로명 확인 메모`;
  const gallerySummary =
    children.length > 0
      ? `연결된 지역 ${children.length}개`
      : "도로명·건물명은 전화로 확인";
  const galleryTerminal = "현재 도로명과 건물명, 가능한 날짜·시각을 전화에서 확인합니다.";
  const sceneIndex = `ADDRESS LEVEL · ${node.segments.length
    .toString()
    .padStart(2, "0")}`;
  const sceneCaption = "받을 주소와 가능한 시간 범위를 먼저 확인합니다.";
  ...
  if (headingViolations.length > 0) {
    throw new Error(
      `HONHYEOL_MASSAGE_REGION_ADJACENT_HEADING_SEMANTIC_DUPLICATE:${node.path}:${headingViolations[0].actionId}`,
    );
  }
  ...
  const renderedSurface: RegionRenderedCopyEntry[] = [
    ...
    entry("final:label", "365일 24시간 전화 접수", "verified-operating-fact"),
    entry("final:heading", "받을 곳과 원하는 이용 항목을 준비해 주세요", "owned-copy"),
  ...
  if (new Set(ids).size !== ids.length) {
    throw new Error(`HONHYEOL_MASSAGE_REGION_RENDER_COPY_ID_DUPLICATE:${node.path}`);
  }
  ...
  return {
    ...
    movements,
    finalBeat: {
      label: "365일 24시간 전화 접수",
      heading: "받을 곳과 원하는 이용 항목을 준비해 주세요",
