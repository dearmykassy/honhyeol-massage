...
- 활성 1,291개 지역 메타를 전수 검사해 메타 3종 고유성, 고객 검색형 지역명, 행정 수식어와 서비스 키워드의 잘못된 결합을 회귀 테스트로 고정한다. Template5 보존 테스트는 이번 승인된 SEO 메타 기준 변경 이후의 콘텐츠·지역 헬퍼 해시를 새 기준으로 삼는다.

## 2026-08-18 KST — 운영 링크·지역 정책 중심 공개 README 정리
...
- 광역 41개와 세부 1,250개의 역할을 목록 도배 없이 설명하고, 근거가 없는 위치 지도·매장 권역·인기 장소·후기·지역별 이용량·이동 및 도착 시간은 만들지 않는 기존 정책을 명시했다. 개발·빌드·중복 감사 명령, GA4 조건부 로드, 이미지 승인 계약은 유지했으며 애플리케이션 코드와 공개 자산은 수정하지 않았다.
...
- 기존 메타데이터 헬퍼, canonical·Open Graph·Twitter·robots, RSS 자동발견 링크와 피드, 브랜드 마크, 지역 이미지 소스는 변경하지 않았다. Template5 바이트 보존 검사도 그대로 통과했다.
- 최종 검증은 Vitest 38/38, `pnpm typecheck`, `pnpm lint`, `pnpm build`, `pnpm audit:build`를 통과했다. 정적 산출물은 공개 페이지·sitemap URL 각 1,299개, 지역 페이지 1,291개, RSS item 2개, 지역 자산 130개와 WebP 390개가 유지됐다.
